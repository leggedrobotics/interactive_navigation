...
def reset_boxes_and_robot(
    ...
    r_min = []
    prev_radius = 0.0
    for i, box_cfg_name in enumerate(boxes_sorted):
        box = env.scene[box_cfg_name]
    ...
    if one_box_only:
        ...
        chain_positions = _random_shift_one_box(
            ...
            chain_positions,
            env_dists,
            torch.tensor(r_min, device=env.device) + min_dist / 2,
            terrain_size,
            step_width,
        )
    ...
    for i, box_cfg_name in enumerate(other_boxes):
        box = env.scene[box_cfg_name]
