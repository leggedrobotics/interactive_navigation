...
def step_terrain(difficulty: float, cfg: mesh_terrains_cfg.StepTerrainCfg) -> tuple[list[trimesh.Trimesh], np.ndarray]:
    ...
    # Create the step
    step_box_dims = (cfg.step_width[0], cfg.step_width[1], step_height)
    step_center = [
        terrain_center[0],
        terrain_center[1],
        step_height / 2,
    ...
    # Origin of the terrain
    # total_height = prev_box_pos[2] + 0.5 * prev_box_height
    origin = np.array([terrain_center[0], terrain_center[1], step_height])
